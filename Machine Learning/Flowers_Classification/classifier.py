...
input_layer = tf.keras.layers.Input([img_h, img_w, img_col])
...
dn1 = tf.keras.layers.Dense(units=512, activation='relu')(flt1)
out = tf.keras.layers.Dense(units=5, activation='softmax')(dn1)


model = tf.keras.Model(input_layer, out)
model.compile(optimizer='adam', loss='sparse_categorical_crossentropy', metrics=['accuracy'])

model.fit(x_train, y_train, batch_size=100, epochs=6000)
